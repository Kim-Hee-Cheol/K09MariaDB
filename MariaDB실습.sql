...
create table member(
  ...
  regidate datetime default CURRENT_TIMESTAMP, /*현재시간을 디폴트로 사용
  																datetime : 날짜와 시간을 동시에 표현할 수 있는 자료형*/
  primary key(id)
);
...
CREATE TABLE tb_int (
  ...
  fnum1 FLOAT(10,5)tb_int NOT NULL,
  fnum2 DOUBLE(20,10)kosmo61_dbkosmo61_db
);
...
/* 자동증가 컬럼에 임의의 값을 삽입할 수 있으나 사용하지 
않는 것이 좋다. */
...
INSERT INTO tb_spec(spec2)
	VALUES('A,B,C');
#설정되지 않은 값으로 입력 시 오류발생
INSERT INTO tb_spec(spec1, spec2)
	VALUES('A','A,B,E');

/*
Model1방식의 게시판을 MariaDB로 컨버팅
*/
-- 전체 레코드 수 조회
...
-- 한페이지에 2개의 게시물이 출력된다고 가정했을 때
/*
	페이지처리를 위한 게시물의 범위를 정할 때 Oracle은 rownum의
